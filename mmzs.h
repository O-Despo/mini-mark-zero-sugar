#define CHECK_BLOCKS    (format_break(&state, in_file, out_file, c)   ||\
                        format_ol_list(&state, in_file, out_file, c) ||\
                        format_ul_list(&state, in_file, out_file, c) ||\
                        format_html(&state, in_file, out_file, c)    ||\
                        format_code(&state, in_file, out_file, c)    ||\
                        format_header(&state, in_file, out_file, c)  ||\
                        format_hr(&state, in_file, out_file, c))

#define CHECK_INLINES   (in_line_emphasis_bold(&state, in_file, out_file, c) ||\
                        in_line_code(&state, in_file, out_file, c)     ||\
                        in_line_link(&state, in_file, out_file, c))
